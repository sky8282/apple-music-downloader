...
func GetStationResp(storefront string, id string, account *structs.Account) (*structs.StationResp, error) {
	...
	obj := new(structs.StationResp)
	err = json.NewDecoder(do.Body).Decode(&obj)
	return obj, err
}

func GetStationAssetsUrl(id string, account *structs.Account) (string, string, error) {
	...
	return "", "", errors.New("no station assets found")
}

func GetStationNextTracks(id string, account *structs.Account) (*structs.AutoGeneratedTrack, error) {
	req, err := http.NewRequest("POST", fmt.Sprintf("https://amp-api.music.apple.com/v1/me/stations/next-tracks/%s", id), nil)
	if err != nil {
		return nil, err
	}
	
	mutoken := strings.TrimSpace(account.MediaUserToken)
	req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", core.DeveloperToken))
	...
	bodyBytes, _ := io.ReadAll(do.Body)
	if do.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("API 状态码异常: %s", string(bodyBytes))
	}

	obj := new(structs.AutoGeneratedTrack)
