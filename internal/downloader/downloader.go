...
func formatAudioQuality(raw string) string {
	if raw == "" {
		return ""
	}
	...
	if re.MatchString(raw) {
		return re.ReplaceAllString(raw, "${1}bit-${2}kHz")
	}
...
func downloadTrackWithFallback(track structs.TrackData, meta *structs.AutoGenerated, albumId, storefront, baseSaveFolder, Codec, covPath string, qobuzDesc string, lyricAccount *structs.Account, workingAccounts []structs.Account, initialAccountIndex int, updateStatus func(status string, sColor func(a ...interface{}) string), progressChan chan runv14.ProgressUpdate, jsonOutput bool, trackNum int, maxAlbumSampleRate float64, m3u8Cache *sync.Map) (string, error) {
	maxRetries := 3
	var lastError error
	...
	for i := 0; i < len(workingAccounts); i++ {
		...
		for attempt := 0; attempt <= maxRetries; attempt++ {
			trackPath, err := downloadTrackSilently(track, meta, albumId, storefront, baseSaveFolder, Codec, covPath, qobuzDesc, lyricAccount, account, progressChan, jsonOutput, maxAlbumSampleRate, m3u8Cache)
			if err == nil {
...
func downloadTrackSilently(track structs.TrackData, meta *structs.AutoGenerated, albumId, storefront, baseSaveFolder, Codec, covPath string, qobuzDesc string, lyricAccount *structs.Account, account *structs.Account, progressChan chan runv14.ProgressUpdate, jsonOutput bool, maxAlbumSampleRate float64, m3u8Cache *sync.Map) (string, error) {
	...
	if needCheck && !needDlAacLc {
		...
		if manifest.Attributes.ExtendedAssetUrls.EnhancedHls != "" && maxAlbumSampleRate > 0 {
			_, defaultRawQuality, _, extErr := parser.ExtractMedia(manifest.Attributes.ExtendedAssetUrls.EnhancedHls, false)
			if extErr == nil && defaultRawQuality != "" {
				re := regexp.MustCompile(`(\d+\.?\d*)kHz`)
				matchDef := re.FindStringSubmatch(defaultRawQuality)
				if len(matchDef) > 1 {
					defRate, _ := strconv.ParseFloat(matchDef[1], 64)
					if defRate >= maxAlbumSampleRate {
						doCheckM3u8 = false
					}
				}
			}
	...
	if core.Dl_atmos {
		TrackQuality = fmt.Sprintf("%dkbps", *core.Atmos_max-2000)
	} else if needDlAacLc {
		TrackQuality = "256kbps"
	} else {
		var rawQuality string
		_, rawQuality, _, err = parser.ExtractMedia(manifest.Attributes.ExtendedAssetUrls.EnhancedHls, true)
		if err != nil {
			if utils.Contains(track.Attributes.AudioTraits, "hi-res-lossless") {
				TrackQuality = "Hi-Res Lossless"
			} else {
				TrackQuality = "Lossless"
			}
		} else {
			TrackQuality = formatAudioQuality(rawQuality)
		}
	}

	var AlbumQuality string
	if core.Dl_atmos {
		AlbumQuality = fmt.Sprintf("%dkbps", *core.Atmos_max-2000)
	} else if core.Dl_aac {
		AlbumQuality = "256kbps"
	} else {
		bestTrackID := track.ID
		if len(meta.Data[0].Relationships.Tracks.Data) > 0 {
			for _, t := range meta.Data[0].Relationships.Tracks.Data {
				if utils.Contains(t.Attributes.AudioTraits, "hi-res-lossless") {
					bestTrackID = t.ID
					break
				}
			}
		...
		if bestTrackID == track.ID {
			AlbumQuality = TrackQuality
		} else {
			bestManifest, err := api.GetInfoFromAdam(bestTrackID, account, storefront)
			if err == nil {
				_, bestRawQ, _, _ := parser.ExtractMedia(bestManifest.Attributes.ExtendedAssetUrls.EnhancedHls, true)
				if bestRawQ != "" {
					AlbumQuality = formatAudioQuality(bestRawQ)
				}
			}
			if AlbumQuality == "" {
				isHires := false
				for _, tr := range meta.Data[0].Relationships.Tracks.Data {
					if utils.Contains(tr.Attributes.AudioTraits, "hi-res-lossless") {
						isHires = true
						break
					}
				}
				if isHires {
					AlbumQuality = "Hi-Res Lossless"
				} else {
					AlbumQuality = "Lossless"
				}
			}
		}
	}

	trackSpecificTags := []string{}
	if track.Attributes.IsAppleDigitalMaster && core.Config.AppleMasterChoice != "" {
		trackSpecificTags = append(trackSpecificTags, core.Config.AppleMasterChoice)
	}
	if track.Attributes.ContentRating == "explicit" && core.Config.ExplicitChoice != "" {
		trackSpecificTags = append(trackSpecificTags, core.Config.ExplicitChoice)
	} else if track.Attributes.ContentRating == "clean" && core.Config.CleanChoice != "" {
		trackSpecificTags = append(trackSpecificTags, core.Config.CleanChoice)
	}
	Track_Tag_String := strings.Join(trackSpecificTags, " ")
	...
	hasClean := false
	for _, t := range meta.Data[0].Relationships.Tracks.Data {
		if t.Attributes.IsAppleDigitalMaster {
			hasMaster = true
		}
		if t.Attributes.ContentRating == "explicit" {
			hasExplicit = true
		}
		if t.Attributes.ContentRating == "clean" {
			hasClean = true
		}
	}
	if hasMaster && core.Config.AppleMasterChoice != "" {
		albumTags = append(albumTags, core.Config.AppleMasterChoice)
	}
	if hasExplicit && core.Config.ExplicitChoice != "" {
		albumTags = append(albumTags, core.Config.ExplicitChoice)
	} else if hasClean && core.Config.CleanChoice != "" {
		albumTags = append(albumTags, core.Config.CleanChoice)
	}
	Album_Tag_String := strings.Join(albumTags, " ")
	...
	currentDiscNum := track.Attributes.DiscNumber
	currentTrackNum := track.Attributes.TrackNumber
	maxDiscNum := 0
	tracksOnCurrentDisc := 0

	for _, t := range meta.Data[0].Relationships.Tracks.Data {
		if t.Attributes.DiscNumber > maxDiscNum {
			maxDiscNum = t.Attributes.DiscNumber
		}
		if t.Attributes.DiscNumber == currentDiscNum {
			tracksOnCurrentDisc++
		}
	}
	...
	if strings.Contains(albumId, "pl.") {
		albumFoldername = strings.NewReplacer(
			"{PlaylistName}", core.LimitString(meta.Data[0].Attributes.Name),
			"{PlaylistId}", albumId, "{Quality}", AlbumQuality, "{Codec}", Codec, "{Tag}", Album_Tag_String,
		).Replace(core.Config.PlaylistFolderFormat)
	} else {
		albumFoldername = strings.NewReplacer(
			"{ReleaseDate}", meta.Data[0].Attributes.ReleaseDate, "{ReleaseYear}", meta.Data[0].Attributes.ReleaseDate[:4],
			...
			"{UPC}", meta.Data[0].Attributes.Upc, "{RecordLabel}", meta.Data[0].Attributes.RecordLabel,
			"{Copyright}", meta.Data[0].Attributes.Copyright, "{AlbumId}", albumId,
			"{Quality}", AlbumQuality, "{Codec}", Codec, "{Tag}", Album_Tag_String,
		).Replace(core.Config.AlbumFolderFormat)
	}

	songName := strings.NewReplacer(
		"{SongId}", track.ID,
		"{SongNumer}", fmt.Sprintf("%02d", currentTrackNum),
	...
	if maxDiscNum > 1 {
		finalAlbumFolder = filepath.Join(finalAlbumFolder, fmt.Sprintf("CD%d", currentDiscNum))
	}

	os.MkdirAll(finalAlbumFolder, os.ModePerm)
	trackPath := filepath.Join(finalAlbumFolder, finalFilename)
	tempTrackPath := trackPath + ".tmp"
	defer func() {
		_ = os.Remove(tempTrackPath)
	}()
	...
	if needDlAacLc {
		...
		_, err := runv3.Run(track.ID, tempTrackPath, core.DeveloperToken, account.MediaUserToken, false, "")
		if err != nil {
		...
		if account.TemplateDecrypt {
			err = runv4.Run(track.ID, trackM3u8Url, tempTrackPath, account, core.Config, progressChan)
			if err != nil {
				return "", fmt.Errorf("failed to run v4 (Wrapper) with account %s: %w", account.Name, err)
			}
		} else {
			err = runv14.Run(track.ID, trackM3u8Url, tempTrackPath, account, core.Config, progressChan)
			if err != nil {
	...
	trackIndexInMeta := trackNum
	var finalLrc string
	if lyricAccount != nil && (core.Config.EmbedLrc || core.Config.SaveLrcFile) {
		lrcStr, lrcErr := lyrics.Get(storefront, track.ID, core.Config.LrcType, core.Config.Language, core.Config.LrcFormat, core.DeveloperToken, lyricAccount.MediaUserToken, core.Config.EnableTranslation, core.Config.TranslationLanguage)
		if lrcErr == nil {
	...
	if qobuzDesc != "" {
		...
		finalComment += "©Copyright Qobuz：" + qobuzDesc
	}

	tags := []string{
		"tool=",
		fmt.Sprintf("artist=%s", meta.Data[0].Attributes.ArtistName),
		fmt.Sprintf("title=%s", meta.Data[0].Relationships.Tracks.Data[trackIndexInMeta-1].Attributes.Name),
		fmt.Sprintf("album=%s", meta.Data[0].Attributes.Name),
		fmt.Sprintf("album_artist=%s", meta.Data[0].Attributes.ArtistName),
		fmt.Sprintf("genre=%s", meta.Data[0].Relationships.Tracks.Data[trackIndexInMeta-1].Attributes.GenreNames[0]),
		fmt.Sprintf("created=%s", meta.Data[0].Attributes.ReleaseDate),
		fmt.Sprintf("writer=%s", meta.Data[0].Relationships.Tracks.Data[trackIndexInMeta-1].Attributes.ComposerName),
	}

	var dNum, dTotal, tNum, tTotal int
	if strings.Contains(meta.Data[0].ID, "pl.") && !core.Config.UseSongInfoForPlaylist {
		dNum, dTotal = 1, 1
		tNum, tTotal = trackNum, len(meta.Data[0].Relationships.Tracks.Data)
	} else {
		trackData := meta.Data[0].Relationships.Tracks.Data[trackIndexInMeta-1]
		lastTrack := meta.Data[0].Relationships.Tracks.Data[len(meta.Data[0].Relationships.Tracks.Data)-1]
		dNum = trackData.Attributes.DiscNumber
		dTotal = lastTrack.Attributes.DiscNumber
		tNum = trackData.Attributes.TrackNumber
		tTotal = tracksOnCurrentDisc
	}
	tags = append(tags, fmt.Sprintf("disk=%d/%d", dNum, dTotal))
	tags = append(tags, fmt.Sprintf("tracknum=%d/%d", tNum, tTotal))

	if meta.Data[0].Attributes.Copyright != "" {
		tags = append(tags, fmt.Sprintf("copyright=%s", meta.Data[0].Attributes.Copyright))
	}

	contentRating := meta.Data[0].Relationships.Tracks.Data[trackIndexInMeta-1].Attributes.ContentRating
	if contentRating == "explicit" {
		tags = append(tags, "rating=1")
	} else if contentRating == "clean" {
	...
	if finalComment != "" {
		tags = append(tags, fmt.Sprintf("comment=%s", finalComment))
	}
	if finalLrc != "" {
		tags = append(tags, fmt.Sprintf("lyrics=%s", finalLrc))
	}

	if core.Config.TagItunesID {
		tags = append(tags, fmt.Sprintf("albid=%s", albumId))
		if len(meta.Data[0].Relationships.Artists.Data) > 0 {
			tags = append(tags, fmt.Sprintf("artid=%s", meta.Data[0].Relationships.Artists.Data[0].ID))
		}
	}
	...
	cmd := exec.Command("MP4Box", "-quiet", "-itags", tagsString, tempTrackPath)
	var stderr bytes.Buffer
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		if !jsonOutput {
			fmt.Printf("\n[警告] MP4Box 执行失败: %v, 输出: %s\n", err, stderr.String())
		}
		return "", fmt.Errorf("元数据写入失败，文件可能不完整: %v", err)
	}
...
func Rip(albumId string, storefront string, urlArg_i string, urlRaw string, jsonOutput bool) error {
	...
	if core.Dl_atmos {
		...
		bestTrackID := meta.Data[0].Relationships.Tracks.Data[0].ID
		for _, t := range meta.Data[0].Relationships.Tracks.Data {
			if utils.Contains(t.Attributes.AudioTraits, "hi-res-lossless") {
				bestTrackID = t.ID
				break
		...
		manifest, err := api.GetInfoFromAdam(bestTrackID, mainAccount, storefront)
		var rawQuality string
		if err == nil {
			needCheckRip := false
			if core.Config.GetM3u8Mode == "all" {
				needCheckRip = true
			} else if core.Config.GetM3u8Mode == "hires" {
				needCheckRip = true
			}
			if needCheckRip {
				for r := 0; r < 5; r++ {
					bestEnhancedHls, _ := parser.CheckM3u8(bestTrackID, "album", mainAccount)
					if strings.HasSuffix(bestEnhancedHls, ".m3u8") {
						manifest.Attributes.ExtendedAssetUrls.EnhancedHls = bestEnhancedHls
						break
					}
					time.Sleep(500 * time.Millisecond)
				}
			}
			_, rawQuality, _, _ = parser.ExtractMedia(manifest.Attributes.ExtendedAssetUrls.EnhancedHls, true)
		}

		if rawQuality != "" {
			Quality = formatAudioQuality(rawQuality)
		} else {
			isHires := false
			isLossless := false
	...
	Tag_string := strings.Join(albumTags, " ")
	...
	var maxAlbumSampleRate float64 = 48.0
	preFetchRates := make(map[int]float64)
	if isHires {
		if !jsonOutput {
			//fmt.Println("快速探测专辑最高音质...")
		}
		var preWg sync.WaitGroup
		var mu sync.Mutex
		for _, trackIndex := range selected {
			trackData := meta.Data[0].Relationships.Tracks.Data[trackIndex-1]
			if utils.Contains(trackData.Attributes.AudioTraits, "hi-res-lossless") {
				preWg.Add(1)
				go func(tIdx int, t structs.TrackData) {
					defer preWg.Done()
					manifest, err := api.GetInfoFromAdam(t.ID, mainAccount, storefront)
					if err == nil && manifest.Attributes.ExtendedAssetUrls.EnhancedHls != "" {
						_, rawQ, _, _ := parser.ExtractMedia(manifest.Attributes.ExtendedAssetUrls.EnhancedHls, false)
						if rawQ != "" {
							re := regexp.MustCompile(`(\d+\.?\d*)kHz`)
							match := re.FindStringSubmatch(rawQ)
							if len(match) > 1 {
								rate, _ := strconv.ParseFloat(match[1], 64)
								mu.Lock()
								if rate > maxAlbumSampleRate {
									maxAlbumSampleRate = rate
								}
								preFetchRates[tIdx] = rate
								mu.Unlock()
							}
						}
					}
				}(trackIndex, trackData)
			}
		}
		preWg.Wait()
		if !jsonOutput {
			//fmt.Printf("\033[33m当前专辑最高采样率: %.1fkHz\033[0m\n", maxAlbumSampleRate)
		}
	}

	var fullQualityTracks []int
	var crippledTracks []int
	isCrippledMap := make(map[int]bool)
	...
	for _, trackIndex := range selected {
		...
		if isHires {
			if !utils.Contains(trackData.Attributes.AudioTraits, "hi-res-lossless") {
				...
				if rate, ok := preFetchRates[trackIndex]; ok && rate < maxAlbumSampleRate {
					isCrippled = true
				}
			}
		} else if isLossless {
			if !utils.Contains(trackData.Attributes.AudioTraits, "lossless") {
				isCrippled = true
			}
		}

		if isCrippled {
			crippledTracks = append(crippledTracks, trackIndex)
			isCrippledMap[trackIndex] = true
		} else {
			fullQualityTracks = append(fullQualityTracks, trackIndex)
		}
	}

	finalSelected := append(fullQualityTracks, crippledTracks...)
	m3u8Cache := &sync.Map{}

	if len(crippledTracks) > 0 {
		go func() {
			var bgWg sync.WaitGroup
			for _, tIdx := range crippledTracks {
				bgWg.Add(1)
				go func(trackIndex int) {
					defer bgWg.Done()
					tData := meta.Data[0].Relationships.Tracks.Data[trackIndex-1]
					for r := 0; r < 5; r++ {
						uiM3u8, _ := parser.CheckM3u8(tData.ID, "song", mainAccount)
						if strings.HasSuffix(uiM3u8, ".m3u8") {
							m3u8Cache.Store(tData.ID, uiM3u8)
							break
						}
						time.Sleep(500 * time.Millisecond)
					}
	...
	if !jsonOutput {
		...
		displayQuality := albumQualityString
		if isHires && maxAlbumSampleRate > 0 {
			displayQuality = fmt.Sprintf("%s 24Bit/%.1fkHz", albumQualityString, maxAlbumSampleRate)
		}

		fmt.Printf("%s %s | %s | %s | %s\n",
			green("音源:"),
			green(displayQuality),
	...
	for _, trackNum := range finalSelected {
		wg.Add(1)
		go func(trackIndexInMeta int) {
			...
			defer func() {
				...
				if r := recover(); r != nil {
					...
					if !jsonOutput && pui != nil {
						pui.Abort(trackIndexInMeta, "程序异常崩溃")
					}
				}
				wg.Done()
			}()
			...
			isCrippled := isCrippledMap[trackIndexInMeta]

			if !jsonOutput && pui != nil {
				totalTracks := len(meta.Data[0].Relationships.Tracks.Data)
				pui.AddTrack(trackIndexInMeta, totalTracks, trackData.Attributes.Name, "", isCrippled)
			}
			...
			for attempt := 1; attempt <= PostDownloadMaxRetries; attempt++ {
				...
				trackPath, err := downloadTrackWithFallback(trackData, meta, albumId, storefront, baseSaveFolder, Codec, covPath, qobuzDesc, lyricAccount, workingAccounts, statusIndex, updateStatus, progressChan, jsonOutput, trackIndexInMeta, maxAlbumSampleRate, m3u8Cache)
				close(progressChan)
...
func RipStation(stationId string, storefront string, urlRaw string, jsonOutput bool) error {
	account, err := core.GetAccountForStorefront(storefront)
	if err != nil {
		return err
	}

	if len(account.MediaUserToken) <= 50 {
		errMsg := "media-user-token 未设置或无效，无法下载电台。"
		if jsonOutput {
			printJSON(stationId, 0, "Station", "", "error", 0, "", errMsg)
		} else {
			fmt.Println(errMsg)
		}
	...
	if !jsonOutput {
		fmt.Printf("开始获取电台 (Station) 信息 : %s\n", stationId)
	}

	stationResp, err := api.GetStationResp(storefront, stationId, account)
	if err != nil || len(stationResp.Data) == 0 {
		return fmt.Errorf("获取电台信息失败: %v", err)
	}
	
	stationName := stationResp.Data[0].Attributes.Name
	isLive := stationResp.Data[0].Attributes.IsLive
	...
	isRadioStation := stationResp.Data[0].Attributes.PlayParams.Kind == "radioStation"
	
	playId := stationResp.Data[0].Attributes.PlayParams.StationHash
	if playId == "" {
		playId = stationResp.Data[0].Attributes.PlayParams.ID
	}
	if playId == "" {
		playId = stationId 
	}
	...
	singerFolder := filepath.Join(baseSaveFolder, "Apple Music Station")
	playlistFolder := core.ForbiddenNames.ReplaceAllString(stationName, "_")
	playlistFolderPath := filepath.Join(singerFolder, playlistFolder)
	os.MkdirAll(playlistFolderPath, os.ModePerm)
	
	covPath, _ := metadata.WriteCover(playlistFolderPath, "cover", stationResp.Data[0].Attributes.Artwork.URL)

	if isLive || isRadioStation {
		trackPath := filepath.Join(playlistFolderPath, fmt.Sprintf("%s.m4a", playlistFolder))
		exists, _ := utils.FileExists(trackPath)
		if exists {
			if !jsonOutput { fmt.Println("电台文件已存在:", trackPath) }
			return nil
		}

		assetsUrl, serverUrl, err := api.GetStationAssetsUrl(stationId, account)
		if err != nil { return fmt.Errorf("获取电台资源链接失败: %v", err) }

		trackM3U8, err := runv3.ResolveStationVariantPlaylist(assetsUrl, core.DeveloperToken, account.MediaUserToken)
		if err != nil { return fmt.Errorf("解析电台变体列表失败: %v", err) }

		keyAndUrls, err := runv3.Run(stationId, trackM3U8, core.DeveloperToken, account.MediaUserToken, true, serverUrl)
		if err != nil { return fmt.Errorf("获取解密密钥失败: %v", err) }

		err = runv3.ExtMvData(keyAndUrls, trackPath)
		if err != nil { return fmt.Errorf("下载电台流失败: %v", err) }

		tags := []string{
			"tool=", "disk=1/1", "track=1", "tracknum=1/1",
			"artist=Apple Music Station", "performer=Apple Music Station", "album_artist=Apple Music Station",
			fmt.Sprintf("album=%s", stationName), fmt.Sprintf("title=%s", stationName),
		}
		if covPath != "" { tags = append(tags, fmt.Sprintf("cover=%s", covPath)) }
		
		cmd := exec.Command("MP4Box", "-itags", strings.Join(tags, ":"), trackPath)
		cmd.Run()

		if !jsonOutput { fmt.Println("\n电台下载完成:", trackPath) }
		return nil

	} else {
		if !jsonOutput {
			fmt.Printf("检测到为算法电台，正在获取曲目推流...\n")
		}
		
		tracksResp, err := api.GetStationNextTracks(stationId, account)
		if err != nil || tracksResp == nil || len(tracksResp.Data) == 0 {
			if !jsonOutput {
				fmt.Printf("原生 ID 获取失败或为空，自动切换防伪 Hash (%s) 重试...\n", playId)
			}
			tracksResp, err = api.GetStationNextTracks(playId, account)
		}

		if err != nil {
			return fmt.Errorf("获取电台曲目失败: %v", err)
		}
		if tracksResp == nil || len(tracksResp.Data) == 0 {
			return errors.New("电台返回的曲目为空 (仍被风控拦截，请检查账号订阅状态或网络)")
		}
		...
		mockMeta := new(structs.AutoGenerated)
		json.Unmarshal([]byte(mockJSON), mockMeta)

		trackBytes, _ := json.Marshal(tracksResp.Data)
		var trackDatas []structs.TrackData
		json.Unmarshal(trackBytes, &trackDatas)
		mockMeta.Data[0].Relationships.Tracks.Data = trackDatas
		...
		workingAccounts := []structs.Account{*account}
		m3u8Cache := &sync.Map{}

		if !jsonOutput {
			fmt.Printf("成功获取到 %d 首电台推流曲目，开始下载...\n", len(trackDatas))
		}

		for i, trackData := range trackDatas {
			trackNum := i + 1
			if trackData.Attributes.Name == "" {
				continue
			}

			_, err := downloadTrackWithFallback(
				trackData, mockMeta, stationId, storefront, baseSaveFolder, Codec, covPath, "",
				account, workingAccounts, 0,
				func(status string, sColor func(a ...interface{}) string) {
					if !jsonOutput {
						msg := status
						if sColor != nil {
							msg = sColor(status)
						}
						fmt.Printf("电台曲目 [%d/%d] %s: %s\n", trackNum, len(trackDatas), trackData.Attributes.Name, msg)
					}
				},
				nil, jsonOutput, trackNum, 48.0, m3u8Cache,
			)

			if err != nil {
				if !jsonOutput {
					fmt.Printf("❌ 曲目 [%d/%d] 下载失败: %v\n", trackNum, len(trackDatas), err)
				}
			} else {
				if !jsonOutput {
					fmt.Printf("✅ 曲目 [%d/%d] 下载完成\n", trackNum, len(trackDatas))
				}
			}
		}

		if !jsonOutput {
			fmt.Println("🎉 算法电台本轮推送曲目下载完成！")
		}
		return nil
	}
}
